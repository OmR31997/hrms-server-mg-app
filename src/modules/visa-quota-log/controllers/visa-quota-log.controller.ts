...
export class VisaQuotaLogController {
    ...
    @Get("/read")
    @Access({resource: "visa-quota-log", action:"create"})
    async get_quota_logs(): Promise<ISuccessResponse<IVisaQuotaLog[]>> {
        const result = await this.visaQuotaLogService.readAll();
        return success("Data fetched successfully", result);
    }

    @Get("/:logId/read")
    @Access({resource: "visa-quota-log", action:"read"})
    async get_quota_log(@Param("logId") logId: string): Promise<ISuccessResponse<IVisaQuotaLog>> {
        const result = await this.visaQuotaLogService.readSingle({ _id: logId });
        return success("Data fetched successfully", result);
    }

    @Delete("/:logId/delete")
    @Access({resource: "visa-quota-log", action:"delete"})
    async delete_quota_log(@Param("logId") logId: string): Promise<ISuccessResponse<IVisaQuotaLog>> {
